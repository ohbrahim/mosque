...
$rolePermissions = [
    ...
    'moderator' => [
        ...
        'view_own_profile',
        'view_own_stats'
        // ملاحظة: لا يوجد delete_any للمشرف
    ],
    'editor' => [
        'add_pages',
        'edit_own_pages',
...
/**
 * التحقق من إمكانية تعديل المحتوى
 */
function canEdit($contentType, $contentOwnerId = null) {
    ...
    // المشرف يمكنه تعديل الصفحات والتعليقات والرسائل
    if ($userRole === 'moderator') {
        return in_array($contentType, ['pages', 'comments', 'messages']);
    }
    ...
    // العضو لا يمكنه تعديل أي محتوى
    return false;
}

/**
 * التحقق من إمكانية الحذف - المدير فقط
 */
function canDelete($contentType, $contentOwnerId = null) {
    if (!isset($_SESSION['user_id'])) {
        return false;
    }
