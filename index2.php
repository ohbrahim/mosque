...
try {
    ...
    if ($activePoll) {
        ...
        $totalVotes = $db->fetchOne("SELECT COUNT(*) as total FROM poll_votes WHERE poll_id = ?", [$activePoll['id']])['total'] ?? 0;
    }
} catch (Exception $e) {
    $activePoll = null;
    $pollOptions = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['vote_option']) && $activePoll) {
    ...
    if (isset($_SESSION['user_id'])) {
        $existingVote = $db->fetchOne("SELECT id FROM poll_votes WHERE poll_id = ? AND user_id = ?", 
                                     [$activePoll['id'], $_SESSION['user_id']]);
        $userVoted = (bool)$existingVote;
    } elseif (isset($_COOKIE['poll_' . $activePoll['id']])) {
        $userVoted = true;
    }
    
    if (!$userVoted && verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        try {
...
if ($activePoll) {
    if (isset($_SESSION['user_id'])) {
        $userVote = $db->fetchOne("SELECT option_id FROM poll_votes WHERE poll_id = ? AND user_id = ?", 
                                 [$activePoll['id'], $_SESSION['user_id']]);
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="?page=contact">اتصل بنا</a>
                    </li>
                </ul>
		...
		<div class="hero-section">
            <div class="container">
                ...
                <div class="mt-4">
                    ...
                    <a href="?page=contact" class="btn btn-outline-light btn-lg">تواصل معنا</a>
                </div>
            </div>
        </div>
    </nav>
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-4">
                    ...
                    <ul class="list-unstyled">
                        ...
                        <li><a href="?page=about"><i class="fas fa-info-circle me-2"></i>عن المسجد</a></li>
                        <li><a href="?page=services"><i class="fas fa-hands-helping me-2"></i>الخدمات</a></li>
                        <li><a href="?page=events"><i class="fas fa-calendar-alt me-2"></i>الفعاليات</a></li>
                        <li><a href="?page=contact"><i class="fas fa-envelope me-2"></i>اتصل بنا</a></li>
                    </ul>
                </div>
                <div class="col-md-4 mb-4">
                    <h5>تواصل معنا</h5>
...
function renderBlock($block) {
    ...
    if ($block['show_title']) {
        // إضافة أيقونة افتراضية إذا لم تكن موجودة
    ...
    switch ($block['block_type']) {
        ...
        // إضافة دعم لشريط الأخبار
        case 'news_ticker':
            $html .= renderNewsTickerBlock($db);
            break;
            
        default:
            $html .= '<p>نوع البلوك غير مدعوم</p>';
            break;
    }
