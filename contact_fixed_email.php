...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message']) && verifyCSRFToken($_POST['csrf_token'] ?? '')) {
    ...
    if (empty($name) || empty($email) || empty($message)) {
        ...
        try {
            ...
            // إرسال بريد إلكتروني للإدارة (فقط في بيئة الإنتاج)
            $adminEmail = getSetting($db, 'admin_email');
            if ($adminEmail && !isLocalEnvironment()) {
                ...
                // محاولة إرسال البريد مع معالجة الأخطاء
                $emailSent = @mail($adminEmail, $emailSubject, $emailBody, "From: $email");
                
                // في حالة فشل الإرسال، لا نعرض خطأ للمستخدم
                // لأن الرسالة تم حفظها في قاعدة البيانات بنجاح
            }
        ...
        <div class="container">
            <?php if (isLocalEnvironment()): ?>
                <div class="dev-notice">
                    ...
                    <strong>ملاحظة للمطور:</strong> أنت في بيئة التطوير المحلية. إرسال البريد الإلكتروني معطل. الرسائل ستُحفظ في قاعدة البيانات فقط.
                </div>
            <?php endif; ?>
